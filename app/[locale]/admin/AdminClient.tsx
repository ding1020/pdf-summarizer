...
export default function AdminClient({
  ...
  return (
    <main className="min-h-[80vh] py-8 px-4 max-w-5xl mx-auto" id="main-content">
      ...
      {tab === "payments" && (
        <>
          {/* Manual payment disabled notice — tab stays visible so admins can
              still review any historically pending requests, but new submissions
              are blocked at the API. */}
          {!manualPaymentEnabled && (
            <div className="mb-4 p-4 bg-amber-50 border border-amber-300 rounded-lg text-amber-800 text-sm flex items-start gap-2">
              <svg className="w-5 h-5 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
              </svg>
              <span>{t("manualPaymentDisabled")}</span>
            </div>
          )}
